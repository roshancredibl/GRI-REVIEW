...
const GRI201EconomicPerformance: React.FC = () => {
  ...
  const { reportId, answers, saveAnswers, handleBackClick } = useReportPage({
    questionnaireId: 'GRI201',
    backPath: 'topic-standards-2025/governance'
  });
  ...
  useEffect(() => {
    const loadQuestionnaire = async () => {
      try {
        setLoading(true);
        setLoadError(null);
        // Load GRI201.json based on the component's stem name
        const result = await loadQuestionnaireFromStem('GRI201', 'GRI', '');
        setQuestionnaire(result.questionnaire);
        setErrors(result.errors);
        if (result.errors.length > 0) {
          console.warn(`Loaded GRI201 questionnaire with ${result.errors.length} errors`);
        }
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Failed to load questionnaire';
        setLoadError(errorMessage);
        console.error('Failed to load GRI201 questionnaire:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleAnswersChange = (newAnswers: Record<string, any>) => {
    console.log(`GRI201 answers updated for report ${reportId}:`, newAnswers);
    ...
    return (
      <div className="page-container">
        <div className="page-header">
          ...
          <div className="page-title">GRI 201: Economic Performance</div>
        ...
        <div className="materials-page">
          <div className="error-container">
            ...
            <p>{loadError}</p>
            <p>Please check if GRI201.json exists and is properly formatted.</p>
          </div>
        </div>
      </div>
    );
        ...
        <Suspense fallback={<div className="loading-spinner">Loading questionnaire...</div>}>
          {questionnaire ? (
            ...
            <div className="loading-container">
              <div className="loading-spinner">Loading GRI 201 questionnaire...</div>
            </div>
          ) : null}
        </Suspense>
      </div>
...
export default GRI201EconomicPerformance;
