...
const GRI301Materials: React.FC = () => {
  ...
  useEffect(() => {
    const loadQuestionnaire = async () => {
      try {
        setLoading(true);
        setLoadError(null);
        // Load GRI301.json based on the component's stem name
        const result = await loadQuestionnaireFromStem('GRI301', 'GRI', '');
        setQuestionnaire(result.questionnaire);
        setErrors(result.errors);
        if (result.errors.length > 0) {
          console.warn(`Loaded GRI301 questionnaire with ${result.errors.length} errors`);
        }
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Failed to load questionnaire';
        setLoadError(errorMessage);
        console.error('Failed to load GRI301 questionnaire:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleAnswersChange = (newAnswers: Record<string, any>) => {
    console.log(`GRI301 answers updated for report ${reportId}:`, newAnswers);
    ...
    return (
      <div className="page-container">
        <div className="page-header">
          ...
          <div className="page-title">GRI 301: Materials</div>
        ...
        <div className="materials-page">
          <div className="error-container">
            ...
            <p>{loadError}</p>
            <p>Please check if GRI301.json exists and is properly formatted.</p>
          </div>
        </div>
      </div>
    );
        ...
        <Suspense fallback={<div className="loading-spinner">Loading questionnaire...</div>}>
          {questionnaire ? (
            ...
            <div className="loading-container">
              <div className="loading-spinner">Loading GRI 301 questionnaire...</div>
            </div>
          ) : null}
        </Suspense>
      </div>
...
export default GRI301Materials;
