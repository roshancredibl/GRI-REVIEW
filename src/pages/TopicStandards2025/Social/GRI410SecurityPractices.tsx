...
const GRI410SecurityPractices: React.FC = () => {
  ...
  useEffect(() => {
    const loadQuestionnaire = async () => {
      try {
        setLoading(true);
        setLoadError(null);
        // Load GRI410.json based on the component's stem name
        const result = await loadQuestionnaireFromStem('GRI410', 'GRI', '');
        setQuestionnaire(result.questionnaire);
        setErrors(result.errors);
        if (result.errors.length > 0) {
          console.warn(`Loaded GRI410 questionnaire with ${result.errors.length} errors`);
        }
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Failed to load questionnaire';
        setLoadError(errorMessage);
        console.error('Failed to load GRI410 questionnaire:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleAnswersChange = (newAnswers: Record<string, any>) => {
    console.log(`GRI410 answers updated for report ${reportId}:`, newAnswers);
      ...
      <div className="page-container">
        ...
        <div className="materials-page">
          <div className="error-container">
            ...
            <p>{loadError}</p>
            <p>Please check if GRI410.json exists and is properly formatted.</p>
          </div>
        </div>
      </div>
    );
        ...
        <Suspense fallback={<div className="loading-spinner">Loading questionnaire...</div>}>
          {questionnaire ? (
            ...
            <div className="loading-container">
              <div className="loading-spinner">Loading GRI 410 questionnaire...</div>
            </div>
          ) : null}
        </Suspense>
      </div>
